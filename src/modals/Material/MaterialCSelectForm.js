...
import {
    ...
    Grid,
    GridItem, HStack, Input, Popover, PopoverArrow, PopoverBody, PopoverContent, PopoverTrigger,
    Radio,
    RadioGroup,
    Stack,
    Text,
...
import {callSimpleMaterialClientsAPI, callSimpleSalesClientsAPI} from "../../apis/ClientAPICalls";
import DaumPostcodeEmbed from "react-daum-postcode";


//*****************************************************************************************
// 시간이 없어서 하드 코딩중인 하드코딩 스크립트 파일입니다. 이후에 리펙토링을 꼭 해주세요
//****************************************************************************************
function MaterialCSelectForm({handleClientType, handleNewClient, handleExistingClient}) {
    ...
    useEffect(() => {
        dispatch(callSimpleMaterialClientsAPI());
    }, [dispatch]);
            ...
            <RadioGroup onChange={setClientType} value={clientType} colorScheme='orange'>
                <Grid templateColumns='repeat(13, 1fr)'>
                    ...
                    <GridItem colSpan={5}>
                        <VStack mt={4} spacing={2} align='left'>
                            <Radio value="existing">
                                <Text fontWeight='600' color='secondaryGray.900'>기존 공급업체 선택</Text>
                            </Radio>
                            <Box overflowY="auto" h="230px" mt={4} pr={4}>
                                ...
                                >
                                    <Stack direction="column">
                                        {simpleClients && simpleClients.data.content.map((client) => (
                                            <Box
                                                as="label"
                    ...
                    <GridItem colSpan={5}>
                        <VStack mt={4} spacing={2} align='left'>
                            <Radio value="new">
                                <Text fontWeight='600' color='secondaryGray.900'>신규 공급업체 등록</Text>
                            </Radio>
                            <FormControl>
...
export default MaterialCSelectForm;